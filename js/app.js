class ShopApp {
    ...
    renderHomePage() {
        ...
        mainContent.innerHTML = `
            ...
            <div class="relative h-[300px] md:h-[500px] mb-8 md:mb-16 overflow-hidden rounded-xl md:rounded-2xl transform hover:scale-[1.02] transition-transform duration-700">
                <div class="absolute inset-0">
                    ${SHOP_DATA.banners.map((banner, index) => `
                        <div class="absolute inset-0 transition-all duration-1500 ease-in-out ${index === 0 ? 'opacity-100' : 'opacity-0'}"
                            ...
                            <div class="absolute inset-0 bg-gradient-to-r from-black/60 via-black/30 to-transparent flex items-center">
                                <div class="container mx-auto px-6">
                                    <div class="max-w-xl animate-fadeIn" style="animation-duration: 1.5s">
                                        ...
                                        <button class="bg-white text-gray-900 px-8 py-4 rounded-full hover:bg-blue-600 hover:text-white transition-all duration-300 transform hover:scale-105 shadow-lg">
                                            Shop Now
                                        </button>
                                    </div>
                                </div>
            ...
            <div class="container mx-auto px-4 mb-8 md:mb-16">
                <h2 class="text-3xl md:text-4xl font-bold mb-6 md:mb-8 text-center">
                    <span class="bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-purple-600 hover:from-purple-600 hover:to-blue-600 transition-all duration-500">
                        Shop by Category
                    </span>
                </h2>
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-8">
                    ${SHOP_DATA.categories.map(category => `
                        <div class="group relative overflow-hidden rounded-2xl shadow-lg transform hover:-translate-y-2 hover:shadow-2xl transition-all duration-500"
                             onclick="app.showCategory(${category.id})">
                        <div class="absolute inset-0 bg-gradient-to-b from-transparent to-black/80 z-10 
                                  group-hover:from-black/20 group-hover:to-black/90 transition-all duration-500"></div>
                        <img src="${category.image}" 
                             alt="${category.name}" 
                             class="w-full h-80 object-cover transform group-hover:scale-110 transition-transform duration-700">
                        <div class="absolute bottom-0 left-0 right-0 p-6 z-20 transform translate-y-2 
                                  group-hover:translate-y-0 transition-all duration-500">
                            <h3 class="text-2xl font-bold text-white mb-2">${category.name}</h3>
                            <p class="text-gray-200 opacity-0 group-hover:opacity-100 transition-all duration-500 delay-100">
                                ${category.description}
                            </p>
                            <button class="mt-4 bg-white/90 text-gray-900 px-6 py-2 rounded-full opacity-0 
                ...
                <div class="container mx-auto">
                    <h2 class="text-3xl md:text-4xl font-bold mb-8 md:mb-12 text-center">
                        <span class="relative inline-block">
                            Featured Products
                            <span class="absolute bottom-0 left-0 w-full h-1 bg-blue-600 transform scale-x-0 
    ...
    renderFeaturedProducts() {
        return SHOP_DATA.products.slice(0, 4).map(product => `
            <div class="group bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-500 p-4 border border-gray-100 
                        overflow-hidden transform hover:-translate-y-1">
                <div class="relative overflow-hidden rounded-xl aspect-square">
                    <img src="${product.images[0]}" 
                         alt="${product.name}" 
                         class="w-full h-full object-contain group-hover:object-cover transition-all duration-700">
                    <div class="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity duration-500 
                        ...
                        <button onclick="app.showProductDetails(${product.id})" 
                                class="bg-white text-gray-900 px-6 py-3 rounded-full hover:bg-blue-600 hover:text-white 
                                       transform hover:scale-105 transition-all duration-300 translate-y-4 
                ...
                <div class="pt-4">
                    <h3 class="text-lg font-semibold mb-2 group-hover:text-blue-600 transition-colors duration-300">
                        ${product.name}
                    </h3>
                    <div class="flex justify-between items-center mb-4">
                        <p class="text-2xl font-bold text-blue-600 transform group-hover:scale-110 transition-transform duration-300">
                            ${this.formatPrice(product.price)}
                        </p>
                        <div class="text-yellow-400 transform group-hover:scale-110 transition-transform duration-300">
                            ${'★'.repeat(5)}
                        </div>
                    </div>
                    <button onclick="app.addToCart(${product.id})" 
                            class="w-full bg-gray-900 text-white px-6 py-3 rounded-full hover:bg-blue-600 
    ...
    startBannerRotation() {
        ...
        // Increase the interval from 5000ms to 8000ms for slower transitions
    ...
    changeBanner(index) {
        ...
        banners.forEach((banner, i) => {
            // Add a slower transition duration
            banner.style.transition = 'opacity 1.5s ease-in-out'; // Changed from default to 1.5s
    ...
    generateInvoice(order) {
        ...
        const invoiceHtml = `
            ...
            <html>
            <head>
                <title>CoSlim - Invoice ${invoiceNumber}</title>
                <style>
                    @media print {
                ...
                <div class="invoice-container">
                    <div class="header">
                        <div class="company-info">
                            <div class="company-name">CoSlim Electronics Hub</div>
                            ...
                            ${order.products.map((item, index) => {
                                ...
                                return `
                                    <tr>
                                        <td>${index + 1}</td>
                                        <td>
                                            ${product?.name || 'Product Not Found'}<br>
                                            <span style="color: #666; font-size: 14px;">
                                                ${product?.description || ''}
                                            </span>
                                        </td>
                                        <td>${item.quantity}</td>
                                        <td class="amount-column">₹${item.price.toLocaleString('en-IN')}</td>
                                        <td class="amount-column">₹${(item.quantity * item.price).toLocaleString('en-IN')}</td>
                                    </tr>
                                `;
                            }).join('')}
                        </tbody>
                    ...
                    <div class="totals-section">
                        <div class="total-row">
                            ...
                            <div class="total-value">₹${order.total.toLocaleString('en-IN')}</div>
                        </div>
                        <div class="total-row">
                            <div class="total-label">Tax Rate</div>
                            <div class="total-value">5.00%</div>
                        </div>
                        <div class="total-row">
                            <div class="total-label">Total</div>
                            <div class="total-value">₹${(order.total * 1.05).toLocaleString('en-IN')}</div>
                        </div>
                        <div class="total-row">
                    ...
                    <div class="terms-section">
                        <div style="margin-bottom: 10px;">Thanks for shopping with us.</div>
                        <div style="font-weight: bold; margin-bottom: 5px;">Terms & Conditions</div>
                        <div>Full payment is due upon receipt of this invoice.<br>
                        Late payments may incur additional charges or interest as per the applicable laws.</div>
                    </div>
                </div>
            </body>
            </html>
        ...
        if (printWindow) {
            ...
            this.showNotification('Please allow pop-ups to view the invoice', 'error');
        }
    }
    ...
    completeOrder(orderData) {
        const newOrder = {
            ...
            paymentMethod: orderData.paymentMethod, // Add this line
            shippingAddress: {
