class AdminPanel {
                            ...
                            <tr>
                                ...
                                <td>
                                    ${order.products.map(p => {
                                        const product = SHOP_DATA.products.find(prod => prod.id === p.productId);
                                        return `${product?.name} (x${p.quantity})`;
                                    }).join('<br>')}
                                </td>
                                <td>₹${order.total}</td>
                                <td>
                    ...
                    <tbody>
                        ${USERS.length > 0 ? USERS.map((user, index) => `
                            <tr>
                                ...
                                <td>${ORDERS.filter(order => order.userEmail === user.email).length} orders</td>
                                <td>
    ...
    showProfileSettings() {
        ...
        adminContent.innerHTML = `
            <div class="max-w-2xl mx-auto">
                <h2 class="text-2xl font-bold mb-8">Profile Settings</h2>
                
                <!-- Change Username -->
                <div class="bg-white rounded-lg shadow-md p-6 mb-6">
                    <h3 class="text-lg font-semibold mb-4">Change Username</h3>
                    <form id="changeUsernameForm" onsubmit="admin.handleChangeUsername(event)" class="space-y-4">
                        <div>
                            <label class="block text-gray-700 mb-2">Current Username</label>
                            <input type="text" value="${this.currentAdmin.name}" disabled
                ...
                <div class="bg-white rounded-lg shadow-md p-6">
                    ...
                    <form id="changePasswordForm" onsubmit="admin.handleChangePassword(event)" class="space-y-4">
                        <div>
    ...
    handleChangeUsername(event) {
        ...
        const newUsername = event.target.newUsername.value.trim();

        if (newUsername === this.currentAdmin.name) {
            this.showNotification('New username must be different from current username', 'error');
            return;
        }

        // Update username in ADMIN_USERS
        const adminIndex = ADMIN_USERS.findIndex(admin => admin.email === this.currentAdmin.email);
        if (adminIndex !== -1) {
            ADMIN_USERS[adminIndex].name = newUsername;
            this.currentAdmin.name = newUsername;
            saveToLocalStorage();
            this.showNotification('Username updated successfully!');
            this.showProfileSettings(); // Refresh the page
        }
    }
    ...
    handleChangePassword(event) {
        ...
        if (currentPassword !== this.currentAdmin.password) {
        ...
        try {
            ...
            const adminIndex = ADMIN_USERS.findIndex(admin => admin.email === this.currentAdmin.email);
            if (adminIndex !== -1) {
                // Update both the array and current admin object
